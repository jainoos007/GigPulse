  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Task Kanban Board</h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">Organize project deliverables and move tasks across stages</p>
        </div>

        <Button
          onClick={() => setIsModalOpen(true)}
          className="gap-2 text-xs sm:text-sm font-semibold bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20"
        >
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
          {[1, 2, 3, 4].map((i) => (
            <Card key={i} className="p-4 space-y-4 min-h-[400px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
          {columns.map((col) => {
            ...
            return (
              <Card
                key={col.key}
                className="p-4 bg-slate-900/50 border-slate-800 space-y-4 min-h-[500px] flex flex-col"
              >
