...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-64 flex-col border-r border-slate-800 bg-slate-950/80 backdrop-blur shrink-0 fixed inset-y-0 z-40">
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-6">
          <div>
            <p className="px-6 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">
              Main Menu
            </p>
            <NavLinks />
          </div>
      ...
      <div className="flex-1 lg:pl-64 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="sticky top-0 z-30 h-16 border-b border-slate-800 bg-slate-950/80 backdrop-blur px-4 sm:px-8 flex items-center justify-between">
              ...
              <SheetContent side="left" className="w-72 p-0 bg-slate-950 border-slate-800">
                <div className="p-6 border-b border-slate-800">
                  <Link href="/dashboard" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}>
                    ...
                    <span className="font-bold text-base tracking-tight text-white">FreelanceFlow</span>
                  </Link>
                </div>
                <div className="py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Popover>
              ...
              <PopoverContent className="w-80 p-4 bg-slate-900 border-slate-800 text-slate-100">
                ...
                <div className="py-3 space-y-3 text-xs">
                  <div className="p-2.5 rounded-xl bg-slate-800/60 space-y-1">
                    ...
                    <p className="text-slate-400 text-[11px]">All CRM modules active and synchronized.</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-800/60 space-y-1">
                    <p className="font-semibold text-slate-200">Welcome {user.firstName}!</p>
                    <p className="text-slate-400 text-[11px]">Manage clients, projects, tasks, and invoices seamlessly.</p>
                  </div>
                </div>
              </PopoverContent>
            </Popover>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative gap-2 px-2 rounded-xl text-left hover:bg-slate-900">
              ...
              <DropdownMenuContent align="end" className="w-56 bg-slate-900 border-slate-800 text-slate-100">
                ...
                <DropdownMenuItem
                  onClick={logout}
                  className="gap-2 text-xs text-red-400 focus:text-red-300 focus:bg-red-500/10 cursor-pointer"
                >
